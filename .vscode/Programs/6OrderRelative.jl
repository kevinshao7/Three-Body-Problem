...
function eval(r, v, dt, t_end)
    ...
    for i in 1:2,j in 1:2 #convert positions and velocities into relative perspective of body 3
        r[i,j]-=r[3,j]
        v[i,j]-=v[3,j]
    end 
    r = r[1:2,:] #discard data of body 3 (should be zero anyway)
    v = v[1:2,:]
    results=hcat(reshape(r,(1,4)),zeros((1,4))) #initialize results array
    ...
    a[1,:] = aij[3] - (m[1]+m[3])*r[1,:]/r3[1] - aij[2] #acceleration of body 1
    a[2,:] = -m[1]*rij/r3[3] - (m[2]+m[3])*r[2,:]/r3[2] - aij[1] #body 2
    ...
    s[1,:] -= sij[1] #-snap31
    s[1,:] -= sij[2] #-snap32
    s[2,:] = -sij[3]*m[1]/m[2] #snap12
    s[2,:] += -sij[2]*m[3]/m[2] #snap13
    s[2,:] -= sij[1] #-snap31
    s[2,:] -= sij[2] #-snap32
    ...
    for t in 0:dt:t_end
        ...
        step +=1
        if step % 100 == 1
            new = hcat(reshape(r,(1,4)),hcat(t,error(r,v,m,m0,sum_mass,e0,a0)))
...
system = plot(results[:,1:2],results[:,3:4],title="System")
energy = plot(results[:,5],results[:,6],title="Energy Error (1e18)")
angular_m = plot(results[:,5],results[:,7],title="Angular Momentum Error (1e18)")
periodicity = plot(results[:,5],results[:,8],title="Max Periodicity Error")
plot(title,system,energy,angular_m,periodicity,layout=(5,1),size=(500,1000))
