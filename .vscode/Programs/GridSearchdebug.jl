...
function phase3_v(r,v,m)#refine velocities
    ...
    @everywhere v_results = zeros(Float128, (1333, 4)) #initialize results
    @everywhere searchtable = search_table() #1331 cases
    @everywhere dt = 1e-3
    @everywhere t_end = 7
    @everywhere resolution = 10

    v_results[1,1:3] = v[body,:]
    v_results[1,1:3] = run(r,v,m,dt,t_end,resolution,r,v)
    v_results[2:1332,1:3] = searchtable
    #search iteration
    for i in 1:443
        ...
        #period ~ 92.8
        coarse2 = remotecall(run,2, r, core2_intv, m, dt,t_end,resolution, r, core2_intv)#coarse simulation
        coarse3 = remotecall(run,3, r, core3_intv, m,  dt,t_end,resolution, r, core3_intv)
        coarse4 = remotecall(run,4, r, core4_intv, m,  dt,t_end,resolution, r, core4_intv)
        ...
        v_results[i+1, 4] = coarse2_p #save periodicity error into results
        v_results[i+444, 4] = coarse3_p
        v_results[i+887, 4] = coarse4_p
    ...
    v_results[1331, 4] = coarse2_p #save periodicity error into results
    v_results[1332, 4] = coarse3_p


    sleep(2)
    row = argmin(v_results[2:1332,4])
    v[body,:] += search_table[row,1:3]/10^(depth+1) #refine position by converging on periodic solution using optimal node
