...
using Distributed #set up parallel processing
...
@everywhere function Relative(r, v, m, dt, t_end,resolution)
    ...
    results=hcat([0],hcat(reshape(r,(1,6)),reshape(v,(1,6)))) #initialize results array
...
#initial conditions: planar figure-eight
...
# Relative(r, v, m, dt, t_end,resolution)
@everywhere function balanced(r, v, m, dt, t_end)
    resolution = convert(Int64, round(((t_end/dt)/1000), digits=0))
    println(resolution)
...
procs(4) #using 4 cores

for  i in 3:6
    t_end=dt*10^i #number of timesteps = 1e3, 1e4, 1e5, 1e6
    for  j in 1:13 #conduct 13*3+1 = 40 tests
    
    
        b = remotecall(balanced,2, r, v, m, dt, t_end)
        c = remotecall(balanced,3, r, v, m, dt, t_end)
        d = remotecall(balanced,4, r, v, m, dt, t_end)
    ...
    println("1e",i,"step balanced done")
    for  j in 1:13
    ...
    println("1e",i,"step converted done")
    dataframe = convert(DataFrame,Results)
    CSV.write("Balanced,Convert(new).csv",dataframe)
end
